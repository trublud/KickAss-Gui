; KickAss Boron Butterfly GUI Wallet Installer for Windows
; Copyright (c) 2017-2019, The KickAss Project
; See LICENSE

[Setup]
AppName=KickAss GUI Wallet
; For InnoSetup this is the property that uniquely identifies the application as such
; Thus it's important to keep this stable over releases
; With a different "AppName" InnoSetup would treat a mere update as a completely new application and thus mess up

AppVersion=0.14.1.0
DefaultDirName={pf}\KickAss GUI Wallet
DefaultGroupName=KickAss GUI Wallet
UninstallDisplayIcon={app}\kickass-wallet-gui.exe
...
LicenseFile=LICENSE
AppPublisher=The KickAss Developer Community
AppPublisherURL=https://getkickass.org
...
; Background info:
; This installer indeed asks for admin rights so the KickAss files can be copied to a place where they have at least
...
; As of version 0.14.1.0, the KickAss .exe files do not carry version info anyway in their .exe headers.
...
; applications somehow. Luckily this is no issue here because ALL files are "private" to KickAss.

Source: "ReadMe.htm"; DestDir: "{app}"; Flags: ignoreversion
Source: "FinishImage.bmp"; Flags: dontcopy

; KickAss GUI wallet exe and guide
Source: "bin\kickass-wallet-gui.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\kickass-GUI-guide.pdf"; DestDir: "{app}"; Flags: ignoreversion

; KickAss CLI wallet
Source: "bin\kickass-wallet-cli.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\kickass-gen-trusted-multisig.exe"; DestDir: "{app}"; Flags: ignoreversion

; KickAss wallet RPC interface implementation
Source: "bin\kickass-wallet-rpc.exe"; DestDir: "{app}"; Flags: ignoreversion

; KickAss daemon
Source: "bin\kickassd.exe"; DestDir: "{app}"; Flags: ignoreversion

; KickAss daemon wrapped in a batch file that stops before the text window closes, to see any error messages
Source: "kickass-daemon.bat"; DestDir: "{app}"; Flags: ignoreversion;

; KickAss blockchain utilities
Source: "bin\kickass-blockchain-export.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\kickass-blockchain-import.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\kickass-blockchain-mark-spent-outputs.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\kickass-blockchain-usage.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\kickass-blockchain-import.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\kickass-blockchain-ancestry.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\kickass-blockchain-depth.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\kickass-blockchain-prune-known-spent-data.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\kickass-blockchain-prune.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\kickass-blockchain-stats.exe"; DestDir: "{app}"; Flags: ignoreversion
...
; Filename: "{app}\kickass-wallet-gui.exe"; Description: "Run GUI Wallet now"; Flags: postinstall nowait skipifsilent
...
begin
  ...
  blockChainDefaultDir := ExpandConstant('{commonappdata}\bitkickass');
  s := 'The default folder to store the KickAss blockchain is ' + blockChainDefaultDir;
  ...
  RegQueryStringValue(HKEY_CURRENT_USER, 'Software\kickass-project\kickass-core', 'blockchainDataDir', blockChainDir);
  if blockChainDir = '' then begin
    blockChainDir := GetPreviousData('BlockChainDir', '');
  end;
  if blockChainDir = '' then begin
    // Unfortunately 'TInputDirWizardDirPage' does not allow empty field, so "propose" KickAss default location
...
begin
  Result := BlockChainDir('') + '\bitkickass.log';
...
begin
  if CurStep = ssPostInstall then begin
    // Re-build "kickass-daemon.bat" according to actual install and blockchain directory used
    SetArrayLength(s, 3);
    s[0] := 'REM Execute the KickAss daemon and then stay with window open after it exits';
    s[1] := '"' + ExpandConstant('{app}\kickassd.exe') + '" ' + DaemonFlags('');
    s[2] := 'PAUSE';
    SaveStringsToFile(ExpandConstant('{app}\kickass-daemon.bat'), s, false); 
  end;
end;
...
[Icons]
; Icons in the "KickAss GUI Wallet" program group
; Windows will almost always display icons in alphabetical order, per level, so specify the text accordingly
Name: "{group}\GUI Wallet"; Filename: "{app}\kickass-wallet-gui.exe";
Name: "{group}\GUI Wallet Guide"; Filename: "{app}\kickass-GUI-guide.pdf"; IconFilename: "{app}\kickass-wallet-gui.exe"
...
Name: "{group}\Utilities\KickAss Daemon"; Filename: "{app}\kickassd.exe"; Parameters: {code:DaemonFlags}
...
Name: "{group}\Utilities\Textual (CLI) Wallet"; Filename: "{app}\kickass-wallet-cli.exe"; WorkingDir: "{userdocs}\KickAss\wallets"
...
Name: "{group}\Utilities\x (Check Default Wallet Folder)"; Filename: "{win}\Explorer.exe"; Parameters: """{userdocs}\KickAss\wallets"""
Name: "{group}\Utilities\x (Check GUI Wallet Log)"; Filename: "Notepad"; Parameters: """{userappdata}\kickass-wallet-gui\kickass-wallet-gui.log"""
Name: "{group}\Utilities\x (Try Daemon, Exit Confirm)"; Filename: "{app}\kickass-daemon.bat"
Name: "{group}\Utilities\x (Try GUI Wallet Low Graphics Mode)"; Filename: "{app}\start-low-graphics-mode.bat"
Name: "{group}\Utilities\x (Try Kill Daemon)"; Filename: "Taskkill.exe"; Parameters: "/IM kickassd.exe /T /F"

; Desktop icons, optional with the help of the "Task" section
Name: "{commondesktop}\GUI Wallet"; Filename: "{app}\kickass-wallet-gui.exe"; Tasks: desktopicon
...
Root: HKCU; Subkey: "Software\kickass-project"; Flags: uninsdeletekeyifempty
Root: HKCU; Subkey: "Software\kickass-project\kickass-core"; Flags: uninsdeletekey
Root: HKCU; Subkey: "Software\kickass-project\kickass-core"; ValueType: string; ValueName: "blockchainDataDir"; ValueData: {code:BlockChainDirOrEmpty};

; Configure a custom URI scheme: Links starting with "kickass:" will start the GUI wallet exe with the URI as command-line parameter
; Used to easily start payments; example URI: "kickass://<address>?tx_amount=5.0"
Root: HKCR; Subkey: "kickass"; ValueType: "string"; ValueData: "URL:KickAss Payment Protocol"; Flags: uninsdeletekey
Root: HKCR; Subkey: "kickass"; ValueType: "string"; ValueName: "URL Protocol"; ValueData: ""
Root: HKCR; Subkey: "kickass\DefaultIcon"; ValueType: "string"; ValueData: "{app}\kickass-wallet-gui.exe,0"
Root: HKCR; Subkey: "kickass\shell\open\command"; ValueType: "string"; ValueData: """{app}\kickass-wallet-gui.exe"" ""%1"""

; Configure a custom URI scheme: Links starting with "kickassseed:" will start the GUI wallet exe with the URI as command-line parameter
; Used to easily hand over custom seed node info to the wallet, with an URI of the form "kickassseed://a.b.c.d:port"
Root: HKCR; Subkey: "kickassseed"; ValueType: "string"; ValueData: "URL:KickAss Seed Node Protocol"; Flags: uninsdeletekey
Root: HKCR; Subkey: "kickassseed"; ValueType: "string"; ValueName: "URL Protocol"; ValueData: ""
Root: HKCR; Subkey: "kickassseed\DefaultIcon"; ValueType: "string"; ValueData: "{app}\kickass-wallet-gui.exe,0"
Root: HKCR; Subkey: "kickassseed\shell\open\command"; ValueType: "string"; ValueData: """{app}\kickass-wallet-gui.exe"" ""%1"""
