...
Core tests take longer than any other KickAss tests, due to the high amount of computational work involved in validating core components.
...
To run only KickAss's core tests (after building):
...
To run only KickAss's crypto tests (after building):
...
```bash
kickassd --regtest --offline --fixed-difficulty 1
```
...
Open the wallet file with `kickass-wallet-rpc` with RPC port 18083. Finally, start tests by invoking ./blockchain.py or ./speed.py
...
To run only KickAss's hash tests (after building):
...
To run only KickAss's performance tests (after building):
...
To run only KickAss's unit tests (after building):
