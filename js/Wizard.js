...
function walletPathExists(directory, filename, isIOS, walletManager) {
    ...
    if(isIOS)
        var path = kickassAccountsDir + filename;
    else
...
function getApproximateBlockchainHeight(_date, _nettype){
    // time of kickass birth 2014-04-18 10:49:53 (1397818193)
    var kickassBirthTime = _nettype == "Mainnet" ? 1397818193 : _nettype == "Testnet" ? 1410295020 : 1518932025;
    ...
    var secondsPerBlock;
    // before kickass's birth
    if (requestedTime < kickassBirthTime){
        console.log("Calculated blockchain height: 0, requestedTime < kickassBirthTime " );
        return 0;
    }
    // time between during v1
    if (requestedTime > kickassBirthTime && requestedTime < forkTime){
        approxBlockchainHeight = Math.floor((requestedTime - kickassBirthTime)/secondsPerBlockV1);
