...
if [ "$platform" == "darwin" ]; then
    BIN_PATH=$BIN_PATH/kickass-wallet-gui.app/Contents/MacOS/
elif [ "$platform" == "mingw64" ] || [ "$platform" == "mingw32" ]; then
    KICKASSD_EXEC=kickassd.exe
fi
...
get_tag
popd
echo "var GUI_KICKASS_VERSION = \"$TAGNAME\"" >> version.js
...
$QMAKE ../kickass-wallet-gui.pro "$CONFIG" || exit
$MAKE || exit 

# Copy kickassd to bin folder
if [ "$platform" != "mingw32" ] && [ "$ANDROID" != true ]; then
cp ../$KICKASS_DIR/bin/$KICKASSD_EXEC $BIN_PATH
fi
